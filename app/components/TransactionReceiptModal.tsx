...
export default function TransactionReceiptModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/50 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full max-w-sm bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden transition-colors">
        {/* Top Decorative bar */}
        ...
        <div className="p-6">
          ...
          <div className="flex flex-col items-center text-center">
            <div className="w-13 h-13 rounded-full bg-emerald-100 dark:bg-emerald-950/60 flex items-center justify-center text-emerald-600 dark:text-emerald-400 mb-3 shadow-sm">
              <CheckCircle size={32} />
            </div>
            <span className="text-[11px] font-bold tracking-widest text-emerald-600 dark:text-emerald-400 uppercase">
              ทำรายการสำเร็จ
            </span>
            <h2 className="text-2xl font-black font-mono text-slate-900 dark:text-white mt-1">
              ฿{formattedAmount} <span className="text-xs text-slate-500 dark:text-slate-400 font-sans">บาท</span>
            </h2>
          ...
          {/* Slip Card */}
          <div className="mt-5 p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700 divide-y divide-slate-200/70 dark:divide-slate-700 text-xs">
            {/* Reference */}
            <div className="flex items-center justify-between py-2">
              <span className="text-slate-500 dark:text-slate-400">เลขอ้างอิง</span>
              <button
                onClick={handleCopy}
                className="flex items-center gap-1.5 font-mono font-bold text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 transition-colors"
              >
            ...
            {transaction.from_account_number && (
              <div className="flex items-center justify-between py-2">
                <span className="text-slate-500 dark:text-slate-400">จากบัญชี</span>
            ...
            {transaction.to_account_number && (
              <div className="flex items-center justify-between py-2">
                <span className="text-slate-500 dark:text-slate-400">ไปยังบัญชี</span>
            ...
            {/* Fee */}
            <div className="flex items-center justify-between py-2">
              <span className="text-slate-500 dark:text-slate-400">ค่าธรรมเนียม</span>
              <span className="font-mono text-emerald-600 dark:text-emerald-400 font-bold">0.00 THB</span>
            </div>

            {/* Description */}
            {transaction.description && (
              <div className="flex items-center justify-between py-2">
                <span className="text-slate-500 dark:text-slate-400">บันทึกช่วยจำ</span>
                <span className="text-slate-800 dark:text-slate-200 truncate max-w-[160px] text-right font-medium">
                  {transaction.description}
                </span>
              </div>
            )}
            ...
            <div className="flex items-center justify-between py-2">
              ...
              <span className="font-mono text-slate-700 dark:text-slate-300">{formattedDate}</span>
            </div>
          </div>
